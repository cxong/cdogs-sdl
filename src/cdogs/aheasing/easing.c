...
{
	return sin(13 * MPI_2 * p) * pow(2, 10 * (p - 1));
}

// Modeled after the damped sine wave y = sin(-13pi/2*(x + 1))*pow(2, -10x) + 1
AHFloat ElasticEaseOut(AHFloat p)
{
	return sin(-13 * MPI_2 * (p + 1)) * pow(2, -10 * p) + 1;
}
...
AHFloat ElasticEaseInOut(AHFloat p)
{
	if(p < 0.5)
	{
		return 0.5 * sin(13 * MPI_2 * (2 * p)) * pow(2, 10 * ((2 * p) - 1));
	}
	else
	{
		return 0.5 * (sin(-13 * MPI_2 * ((2 * p - 1) + 1)) * pow(2, -10 * (2 * p - 1)) + 2);
	}
}
...
AHFloat BackEaseInOut(AHFloat p)
{
	...
	{
		AHFloat f = 2 * p;
		return 0.5 * (f * f * f - f * sin(f * MPI));
	}
	else
	{
		AHFloat f = (1 - (2*p - 1));
		return 0.5 * (1 - (f * f * f - f * sin(f * MPI))) + 0.5;
	}
}
